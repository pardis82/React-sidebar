    ...
    >
      ...
      >
        ...
        <InputType
          ...
          className={clsx(
            "w-full bg-transparent outline-none text-[14.5px] leading-normal font-sans text-right ",
        ...
        {label && (
          <label
            htmlFor={inputId}
            className={clsx(
              "absolute transition-all duration-150 px-1 font-sans pointer-events-none",
              "right-2",
              float
                ? clsx(
                    "text-[15px]",
      ...
      {/* Error message */}
      {errorMessage ? (
        <p className="text-red-500 font-sans text-sm mt-1">{errorMessage}</p>
      ) : helperText ? (
        <p className="text-gray-500 font-sans text-sm mt-1">{helperText}</p>
      ) : null}
    </div>
  );
}
